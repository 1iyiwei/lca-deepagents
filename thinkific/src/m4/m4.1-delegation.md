...
<p align="center">
  <img src="images/m4.1-two-capabilities.svg" alt="A two-panel drawing. Left: a to-do list with checkboxes (two checked, one in progress, one pending) labeled Planning. Right: a small robot supervisor connected to three robot subagents, labeled Subagents." width="720">
</p>

- **Planning**: `write_todos` keeps a visible task list so the main agent can track what is pending, in progress, and complete.
...
- **Planning** keeps the main agent organized with a visible `write_todos` task list.
...
<MCQ
    ...
    correctIndex={0}
    explanation="The planning tool maintains a structured to-do list in the agent’s state, so the agent has an explicit plan to track against on a long, multi-step job. Across turns, that state persists when you use the same thread with a checkpointer."
/>
