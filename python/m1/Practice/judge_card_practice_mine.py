...
trophies.""" + TOOL_SEQUENCE,
    ...
    "sweet_chef": """You are Dulce, a sweet pastry chef with a theatrical,
  gourmand's air and a cookbook's precision. Your job is the same as the other judges:
  score three personality traits, match the result to a product, and hand off a
  final verdict — but you do it as if assembling a signature dessert.
  Speak in sensory, culinary metaphors: treat each trait as an ingredient
  (a pinch of chaos, a dollop of daring, a drizzle of collaboration) and
  describe the resulting personality as the finished pastry. Be warm,
  playful, and slightly indulgent — offer charming, food-forward imagery
  ("like a tart with a fiery raspberry coulis"), elegant plating metaphors,
  and a final recommendation phrased as a menu suggestion or pairing.
  Never be clinical or terse: fold personality into prose as you would
  fold meringue, and end every verdict with a succinct product match and
  one-sentence serving suggestion that makes the user smile.""" + TOOL_SEQUENCE,
}
...
def score_and_match(answers: list[tuple[int, int, int]]) -> dict:
    ...
    direction = TRAIT_AXES[axis_index][0 if scores[axis_index] < 50 else 1]
...
async def mcp_helper(product: str):
    ...
    result = await agent.ainvoke({
        "messages": [{"role": "user", "content": f"Use the LangChain docs MCP tool to explain what {product} is."}]
    })

    return get_content(model, result["messages"][-1])
...
def fetch_product_fact(product: str) -> str:
    ...
    return asyncio.run(mcp_helper(product))
...
JUDGES_TO_RUN = ["salty_pirate", "savage_critic", "ancient_mummy", "sweet_chef"]  # TODO 4: e.g. ["your_persona", "ancient_mummy"]
