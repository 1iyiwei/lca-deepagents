...
@tool
def order_lunch(food: str) -> str:
    """Place a lunch order with the given argument."""
    return f"Lunch order placed: {food!r}"
...
SYSTEM_PROMPT = """You are a helpful agent to place lunch order for me while I am busy at work.
Rules:
- You will ask me for my preference by calling order_lunch.
- You proceed to place the order only after you receive my response.
"""
INITIAL_REQUEST = "I am hungry. Can you order a steamed code for me?"
INTERRUPT_ON = {"order_lunch": {"allowed_decisions": ["approve", "edit", "reject"]}}

if "TODO 1" in order_lunch.description:
...
for msg in result.value["messages"]:
    if hasattr(msg, "name") and msg.name == "order_lunch":
        print(msg.content)
        break
