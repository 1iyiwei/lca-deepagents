...
thread_a = {"configurable": {"thread_id": "m1.7_homework_a"}}
thread_b = {"configurable": {"thread_id": "m1.7_homework_b"}}
...
def run_scenario():
    statement = "My favorite snack is pure dark chocolate."
    question = "What is my favorite snack?"

    # thread a, statement and question
    result = agent.invoke(
    ...
    # second agent with fresh memory and invoke it on thread_a, question only
    fresh_agent = create_deep_agent(model=model, checkpointer=MemorySaver())
    result = fresh_agent.invoke(
        {"messages": [{"role": "user", "content": question}]},
        config=thread_b,
