...
FOLDER = "m2.2_homework"
my_dir = Path(__file__).parent / FOLDER
my_dir.mkdir(exist_ok=True)
COMMON_CONTENT = "I am doing the homework for m2.2 backends. I can create my own task and choose different backend options, including storage (in memory state or file) and access options (read or write). I plan to have a default for state, and two files with different access permissions."
READ_FILENAME = "read.md"
WRITE_FILENAME = "write.md"
(my_dir / READ_FILENAME).write_text(COMMON_CONTENT + "This one is read only.")
(my_dir / WRITE_FILENAME).write_text(COMMON_CONTENT + "This one can be read and written to.")

backend = CompositeBackend(
    default = StateBackend(),
    routes = {
            f"/{FOLDER}/": FilesystemBackend(
                root_dir=str(my_dir),
                virtual_mode=True,
            ),
    },
...
TASK = f"""
Summarize the content of /{FOLDER}/{READ_FILENAME}, and add an additional thought directly into the file.
Do similar things for /{FOLDER}/{WRITE_FILENAME}.
Finally, repot back what you have done for both files.
"""
#permissions: list[FilesystemPermission] = []  # TODO 2 (optional): add rules here
permissions=[
    FilesystemPermission(
        operations=["write"],
        paths=[f"/{FOLDER}/{READ_FILENAME}"],
        mode="deny",
    ),
]
